...
def billing():
    user = get_user_details(session['user_email'])
    get_billing = open("billing.json").read()
    billing_data = [x for x in json.loads(get_billing)["patients"] if x["patient_id"] == "12345"][0]["billing"]
    if user:
        appointments = get_user_appointments(session['user_email'])
...
def prescription():
    ...
    prescriptions = open("prescriptions.json")
    prescriptions = prescriptions.read()
    print(prescriptions)
    prescriptions_json = [x for x in json.loads(prescriptions)['patients'] if x['patient_id'] == "12345"][0]
    if user:
        appointments = get_user_appointments(session['user_email'])
        return render_template('billing.html',
                               user=user,
                               ...
                               doctors=DOCTORS,
                               time_slots=TIME_SLOTS,
                               data=prescriptions_json['prescriptions']
                               )
